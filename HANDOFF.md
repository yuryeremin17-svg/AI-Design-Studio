...
> Дата: 2026-03-22 | Сессия: #20

## Что сделано в сессии #20

### Telegram-бот — починен и доработан

**Проблема:** бот не отвечал. Worker задеплоен, webhook установлен, но sendMessage молча не срабатывал.

**Причина:** секрет `BOT_TOKEN` в Cloudflare Workers был повреждён/пустой. Диагностика через `/debug` endpoint → `hasToken: false`. Пересоздание через `wrangler secret put` починило.

**Доработки (v2):**

| # | Что сделано |
|---|-------------|
| 1 | Кнопка «Цены и сроки» на стартовом экране |
| 2 | Цены с долларовым эквивалентом (~$1,350 / ~$2,700 / ~$4,900) |
| 3 | Сроки по пакетам (5-7 / 10-14 / 14-21 дней) |
| 4 | Ключевые слова: срок/время/когда/долго/быстро |
| 5 | Fallback: «передали дизайн-команде, ответим в течение часа» |
| 6 | Обработка фото/документов/голоса — пересылка админу + ответ клиенту |
| 7 | Баннер (OG-image) при /start через sendPhoto |
| 8 | Кнопка «← В начало» на каждом экране |
| 9 | setMyDescription + setMyShortDescription + setMyCommands (/start = Главное меню) |
| 10 | «менеджер» → «дизайн-команда» / «мы» |
| 11 | Аватарка бота 512x512 (рубИИльник Design Studio) — bot/avatar-512.png |
| 12 | Callback кнопки: sendMessage вместо editMessage (фикс — editMessageText не работает на фото) |

**Баг с кнопками:** /start отправляет фото (sendPhoto), а callback пытался editMessageText — но у фото `caption`, не `text`. Заменено на sendMessage (новое сообщение).

**Аватарка:** отправлена в бот, нужно переслать @BotFather → /setuserpic.

### Что осталось (следующие сессии)

**Завтра:**
- [ ] Ответ клиенту через reply от админа (п.17)
- [ ] Бриф через Worker — токен из клиентского JS (п.19-20, безопасность)

**На неделе:**
- [ ] EN/RU переключение языка (п.9)
- [ ] Мини-квалификация кнопками перед брифом (п.13)
- [ ] Напоминание если клиент не заполнил бриф за 24ч (п.14)

### Birca Kolet
- Без изменений, ждём фидбэк Максима (раунд 2)

### Bloshka Deluxe
- Без изменений, ждём показ клиенту через WhatsApp
