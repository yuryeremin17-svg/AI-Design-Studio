...
> Дата: 2026-04-24 | Сессия: #21

## Что сделано в сессии #21

### Гайд «Claude Design & AI Design Studio» — для Ларисы

**Контекст:** 17 апреля 2026 Anthropic запустили Claude Design (веб-инструмент «пиши текст — программа рисует»). Юрий попросил подготовить материал для Ларисы (жена, менеджер медкомпании, занимается разработками по оздоровлению) — объяснить что это, как поставить, когда использовать в её работе.

**Итерации:**
1. **v0 (текст)** — базовый ответ с описанием + сравнение со студией
2. **v1 (HTML для клиента)** — лендинг на 1290 строк, Cormorant + DM Mono + Raleway, тёмно-кремовая палитра, структура «Что это → Как начать → Что делаем мы → Пакеты → Сравнение → Когда что → CTA → Шпаргалка»
3. **v2 (HTML для Ларисы)** — переработано под партнёра: убраны блок «Что делаем мы», пакеты с ценами, CTA «Написать нам», аналогия «ужин в ресторане»; обращение на «ты», имя «Лара». Заменён контекст мокапа с кофейни на wellness (Детокс 21 день, 3 этапа). Добавлены 5 карточек-сценариев с готовыми промптами под задачи медкомпании: презентация программы, листовка, пост, инфографика, афиша.
4. **PDF-экспорт** — 604 КБ (первый вариант был 25 МБ из-за noise-overlay + deviceScaleFactor 2; оптимизирован: отключены анимации, noise, opacity forced to 1).
5. **Отправлено** в Telegram → Ларисе.

**Технические детали:**
- Финальный HTML: ~700 строк, тот же design system что v1, но короче на 25%
- PDF через Chrome headless `page.pdf()`, векторный, без полей
- Мокап слайда перекрашен: синий (кофе) → зелёный градиент (wellness)

### Архивирование
- Создана `archive/2026-04-24_claude-design-guide/` в проекте — все артефакты сессии:
  - `claude-design-larisa.html` + `Claude-Design-Larisa.pdf` (финал)
  - `v1-client-version.html` (первая версия, не использована)
  - `scripts/` — 7 временных скриптов (review-*, pdf-larisa, hero-check, verify-pdf)
  - `duplicates/` — идентичные по MD5 копии PDF+HTML, которые Telegram сохранил в Downloads при отправке
- `.gitignore` обновлён: `archive/` — рабочий мусор сессий не уедет в публичный репо
- Downloads и `~/Documents/Inbox/` — чистые

### Проверка инфраструктуры
По запросу Юрия проверено что бот и приём брифов работают:
- **Worker** `ai-design-studio-bot.yuryeremin17.workers.dev` — HTTP 200 «Bot running»
- **Бриф** `yuryeremin17-svg.github.io/AI-Design-Studio/templates/brief/brand-brief.html` — HTTP 200
- **Webhook** — установлен в сессии #20, после этого `bot/` не менялся (git log пустой) → активен
- **Цепочка бот → бриф → Web3Forms → email** — целая

### Уроки / правила процесса
- **Партнёр ≠ клиент.** Первая версия лендинга была «продающая» — для партнёра (кто уже в теме) нужно убирать пакеты/цены/CTA, использовать «ты», давать практичные готовые промпты под его контекст.
- **PDF из премиум-HTML:** deviceScaleFactor 1 + убрать noise + убрать background animations + forced opacity:1 для forwards-animated элементов = экономия 97% размера без потери читаемости.
- **Telegram + HTML: не отправлять файлами.** Либо PDF, либо ссылка (GitHub Pages). PDF предпочтительнее когда приватно («Лара, привет» не в публичном интернете).

### Что осталось (следующие сессии)
Из #20 переходит без изменений:
- [ ] Ответ клиенту через reply от админа (п.17 бот)
- [ ] Бриф через Worker — токен из клиентского JS (п.19-20, безопасность)
- [ ] EN/RU переключение языка
- [ ] Мини-квалификация кнопками перед брифом
- [ ] Напоминание если клиент не заполнил бриф за 24ч

**Рекомендация:** сохранить `BOT_TOKEN` в 1Password/KeePass — сейчас он только в Cloudflare Secrets, если Worker случайно удалится — токен пропадёт.

### Клиенты
- **Birca Kolet** — без изменений, ждём фидбэк Максима
- **Bloshka Deluxe** — без изменений, ждём показ клиенту
